...
export default function LumpsumYearlyBreakdown({ yearlyData }) {
    ...
    <div className="glass-card-static sip-breakdown-card">
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <div>
            <h4 className="breakdown-title">Yearly Growth Breakdown</h4>
              ...
              <tbody>
                {yearlyData.map((row) => (
                  <tr key={row.year}>
                    ...
                    <td>{formatIndianCurrency(row.initialInvestment)}</td>
                    <td className="font-bold text-navy">
                      {formatIndianCurrency(row.futureValue)}
                    </td>
                    <td className="text-teal">+{formatIndianCurrency(row.estimatedReturns)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
