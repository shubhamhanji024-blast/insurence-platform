...
export default function SIPGrowthChart({ yearlyData }) {
  ...
  const points = yearlyData.map((d, index) => {
    ...
    const yInvested = height - padding.bottom - (d.invested / maxFV) * chartHeight;
    ...
    <div className="glass-card-static sip-growth-chart-card">
      <div className="growth-chart-header">
        <div>
          ...
          <p className="growth-chart-sub">
            Visualizing wealth creation over {yearlyData.length} years
          </p>
        </div>
      ...
      {activePoint && (
        <div className="chart-active-tooltip">
          ...
          <span className="tooltip-stat">
            Invested: <strong>{formatIndianCurrency(activePoint.data.invested)}</strong>
          </span>
          <span className="tooltip-stat">
            Maturity Value: <strong className="text-teal">{formatIndianCurrency(activePoint.data.futureValue)}</strong>
          </span>
          <span className="tooltip-stat">
            Returns: <strong className="text-gold">+{formatIndianCurrency(activePoint.data.returns)}</strong>
          </span>
        </div>
      )}
      ...
      <div className="svg-responsive-container">
        <svg viewBox={`0 0 ${width} ${height}`} className="growth-svg">
          ...
          {/* Filled Area under Portfolio Value */}
          <path d={valueAreaPath} fill="url(#tealGradient)" />

          {/* Invested Amount Line (Navy Dashed) */}
          <path
            d={investedLinePath}
            fill="none"
            stroke="#1e3a8a"
          ...
          {/* Portfolio Value Line (GrowthNest Teal) */}
          <path
            d={valueLinePath}
            fill="none"
            stroke="#19C3A3"
          ...
          {/* Interactive Hover Dots & Vertical Line */}
          {points.map((p, idx) => (
            <g key={idx}>
              <circle
                cx={p.x}
              ...
              {/* Invisible Hit Targets for smooth hover */}
              <rect
